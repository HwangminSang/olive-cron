...
const issueSignUpCode = async (req: Request, res: Response, next: NextFunction) => {
  ...
  let issuedCount = (await verifyCnt(req, res, verifyKey)) || 0
  ...
  vonageVerify.verify.request(
    ...
    async (err: any, result: any) => {
      if (err) {
        ...
        log.info(`result :${result} in issueSignUpCode `)
        log.err('Concurrent verifications to the same number are not allowed in issueSignUpCode')
        waitNextErr.data = {
          issuedCount: issuedCount || 0,
        }
...
export default issueSignUpCode
